...
// state is the server state for a combined directory and store server that
// serves a file system for interacting with the Cloud DNS service that does
// DNS resolution for the upspin.services domain.
...
// server is a wrapper around state that provies the upspin.DirServer and
// upspin.StoreServer methods. A new wrapper is created by each Dial.
type server struct {
