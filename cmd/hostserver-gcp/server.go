...
// that serves a file system for interacting with the Cloud DNS service that
// serves DNS for the upspin.services domain.
// See the package comment for a full description of the file system.
type server struct {
	user upspin.UserName // Set by Dial.

	// state is a pointer to a state struct shared by all instances of the
	// server. It is done this way so that dial can create a *server value
	// for each user of the server.
	*state
}

// state is the server's state, shared among all instances of the server.
type state struct {
	ep  upspin.Endpoint
	cfg upspin.Config
...
// packHost packs a file for the given user's host that contains their ip and
// host, and adds the resulting entry to the cache.
...
// lookup returns the entry for the given user. It first consults the cache. If
// no entry—nor absence of entry—is found there, it invokes lookupName to find
// the user's host information, packs the resulting data, and updates the cache.
